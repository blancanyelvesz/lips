"""
This script calculates the perplexity of text files using the pre-trained language model OpenLLaMA available in huggingface 🤗 for contexts of sizes 10, 20, 30, 40, and 50.
...
model = AutoModelForCausalLM.from_pretrained(model_name, device_map = "auto", torch_dtype = torch.float16 if device.type == "cuda" else torch.float32)
